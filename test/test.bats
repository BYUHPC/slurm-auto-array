...
@test ":::, :::+, ::::, and ::::+ work" {
    ...
    echo i >  input2.txt
    echo j >> input2.txt
    submit_job "$(echo this is stdin)" --wait -o "testcolons-%3 %4 %5 %6 %7.out" --verbose -- \
               bash -c 'while read arg; do echo -n "$arg"; done; echo "$0 $@"' \
               ::: I II III :::+ a b c d ::: x y ::::+ input1.txt :::: input2.txt
    for x in "I a" "II b" "III c"; do
        for y in "x 1" "y 2"; do
            for z in i j; do
                test "$(cat "testcolons-$x $y $z.out")" = "this is stdin$x $y $z"
            done
        done
    done
}